...
namespace ark {
	...
	std::vector<float> AzureKinectCamera::getColorIntrinsics() {
		return std::vector<float>{(float)calib_intrin[0], (float)calib_intrin[2], (float)calib_intrin[1], (float)calib_intrin[3]};
	}

	void AzureKinectCamera::update(MultiCameraFrame & frame) {
		...
		while (depth_image == NULL) {
			...
			frame.images_[1] = xyzMap.clone();
			frame.images_[2] = rgbMap.clone();
			return;
		}
		...
		if (scale = 1.0) {
			frame.images_[2] = rgb_map_large;
		}
		else {
			cv::resize(rgb_map_large, frame.images_[2], frame.images_[2].size(), 0., 0., CV_INTER_CUBIC);
		}
	...
	void AzureKinectCamera::project(cv::Mat & xyz_map) {
	...
	void AzureKinectCamera::imuReader() {
		while (!kill) {


			k4a_imu_sample_t imusample = NULL;
			k4a_device_get_imu_sample(this->k4a_device, &imusample);

			auto ts_g = imusample.acc_timestamp_usec;
			if (last_ts_g != ts_g) {
				last_ts_g = ts_g;
				ImuPair imu_out{ double(ts_g)*1e6, //convert to nanoseconds, for some reason gyro timestamp is in centiseconds
					Eigen::Vector3d(imusample.gyro_sample.x, imusample.gyro_sample.y, imusample.gyro_sample.z),
					Eigen::Vector3d(imusample.acc_sample.x, imusample.acc_sample.y, imusample.acc_sample.z) };
				imu_queue_.enqueue(imu_out);
			}
		}
	}

	bool AzureKinectCamera::getImuToTime(double timestamp, std::vector<ImuPair>& data_out) {
		ImuPair imu_data;
